...
<form method="post" action="<?= POST_FORM_ACTION_URI ?>">
    ...
    <label for="CALENDAR_TYPE_XML_ID"><?= Loc::getMessage('YLAB_MEETING_ADD_COMPONENT_CALENDAR_TYPE_XML_ID') ?> </label>
<!--    <p><input type="string" name="CALENDAR_TYPE_XML_ID"></p>-->
    <select name="CALENDAR_TYPE_XML_ID" id="integr">
        <?php
        foreach ($arResult['CALENDAR_TYPES'] as $calendar) {
            ?>
            <option value="<?= $calendar['XML_ID'] ?>"><?= $calendar['NAME'] ?></option>
            <?php
        }
        ?>
    </select>
