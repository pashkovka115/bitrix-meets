...
<?php else: ?>
    ...
    <?php if (!empty($arParams['ORM_NAME']) && is_array($arParams['COLUMN_FIELDS'])
      && !empty($arParams['LIST_ID']) && !empty($arParams['COLUMN_FIELDS'])) : ?>
        ...
        <?= $arResult['ADD_SUCCESS_NAME'] ? Loc::getMessage('YLAB_MEETING_LIST_ADD_SUCCESS_PT_1') .
          $arResult['ADD_SUCCESS_NAME'] . Loc::getMessage('YLAB_MEETING_LIST_ADD_SUCCESS_PT_2')
          : null ?>
